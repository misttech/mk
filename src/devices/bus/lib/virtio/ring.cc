...
zx_status_t Ring::Init(uint16_t index, uint16_t count) {
  LTRACEF_LEVEL(2, "%s: index %u, count %u", __func__, index, count);

  // check that count is a power of 2
  if (!cpp20::has_single_bit(count)) {
    TRACEF("ring count: %u is not a power of 2", count);
  ...
  if (count > max_ring_size) {
    TRACEF("ring init count too big for hardware %u > %u", count, max_ring_size);
  ...
  TRACEF("%s: need %zu bytes", __func__, dma_buffer_size);
  ...
  /* initialize the ring */
  // vring_init(&ring_, count, ring_buffer_->virt(), PAGE_SIZE);
...
void Ring::FreeDesc(uint16_t desc_index) {
  TRACEF("%s: index %u free_count %u", __func__, desc_index, ring_.free_count);
...
void Ring::SubmitChain(uint16_t desc_index) {
  TRACEF("%s: desc %u", __func__, desc_index);
