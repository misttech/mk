...
zx_status_t pci_add_subtract_io_range(uint32_t mmio, uint64_t base, uint64_t len, uint32_t add) {
...
zx_status_t kernel_pci_init(zx_pci_init_arg_t* arg, uint32_t len) {
  if (!Pci::KernelPciEnabled()) {
    return ZX_ERR_NOT_SUPPORTED;
  }
...
static zx_status_t find_pcie_config(zx_pci_init_arg_t* arg) {
  ...
  int num_buses = table_start->end_bus - table_start->start_bus + 1;

  if (table_start->segment != 0) {
    dprintf(CRITICAL, "Non-zero segment found\n");
    return ZX_ERR_NOT_SUPPORTED;
  }

  arg->addr_windows[0].cfg_space_type = PCI_CFG_SPACE_TYPE_MMIO;
  arg->addr_windows[0].has_ecam = true;
  arg->addr_windows[0].bus_start = table_start->start_bus;
  arg->addr_windows[0].bus_end = table_start->end_bus;
  ...
  arg->addr_windows[0].base =
      table_start->base_address + size_per_bus * arg->addr_windows[0].bus_start;
 ...
 * @return ZX_OK on success
 */
static zx_status_t find_pci_legacy_irq_mapping(const acpi_lite::AcpiParserInterface& parser, zx_pci_init_arg_t* arg) {
 ...
 * @return ZX_OK on success.
 */
static zx_status_t find_pci_config(const acpi_lite::AcpiParserInterface& parser,
                                   zx_pci_init_arg_t* arg) {
 ...
 * @return ZX_OK on success
 */
zx_status_t get_pci_init_arg(const acpi_lite::AcpiParserInterface& parser, zx_pci_init_arg_t** arg,
                             uint32_t* size) {
  ...
  if (status != ZX_OK) {
    status = find_pci_config(parser, res);
    if (status != ZX_OK) {
      goto fail;
    }
  }
...
// This pci_init initializes the kernel pci driver.
void pci_init(uint level) {
  zx_status_t status;

  // TODO (Herrera) : Use APICA or implement in acpi_lite a way to enumerate the PCI devices

  // Add the PIO range for the PCI configuration space
  status = pci_add_subtract_io_range(false /* mmio */, 0x0000, 0xFFFF, true /* add */);
  if (status != ZX_OK) {
  ...
  zx_pci_init_arg_t* arg;
  uint32_t arg_size;
  status = get_pci_init_arg(GlobalAcpiLiteParser(), &arg, &arg_size);
  if (status != ZX_OK) {
    dprintf(CRITICAL, "acpi: error %d in get_pci_init_arg", status);
  }

  status = kernel_pci_init(arg, arg_size);
  if (status != ZX_OK) {
    dprintf(CRITICAL, "acpi: error %d in pci_init", status);
  }

  free(arg);
}

LK_INIT_HOOK(x86_pcie_bus, pci_init, LK_INIT_LEVEL_PLATFORM)
